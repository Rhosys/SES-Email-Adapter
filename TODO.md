...
- [ ] **Calendar RSVP reminders** — when a calendar signal with METHOD:REQUEST has no corresponding user RSVP signal after a configurable period (e.g. 24 hours before event start), surface a reminder notification to the user. Requires: scheduled job that queries calendar signals without a user response, push notification or in-app nudge, respects notification preferences.
- [ ] **Auto-block simulated phishing / "think before you click" security training emails** — corporate security awareness platforms (KnowBe4, Proofpoint Security Awareness, Cofense, Terranova, etc.) send fake phishing emails to employees as training exercises. These are intentionally deceptive and provide zero value to the recipient — they should be silently blocked rather than cluttering the inbox or triggering auto-reply/forwarding rules. Detection signals: `X-Phishtest`, `X-KnowBe4`, `X-PSAT`, `X-Cofense` headers; sender domains on known training-platform lists (e.g. `knowbe4.com`, `phishingtest.com`, `training.knowbe4.com`); subject-line patterns common to these campaigns. Block at signal ingestion with `blockReason: "phish_simulation"` and a dedicated disposition. Include an account-level toggle to disable this if the user runs their own awareness program and wants to see the signals.
...
- [ ] **Validate outbound sender domain is owned by the account** — before sending any email (reply, forward, auto-reply, invite, onboarding), verify that the `from` address domain is a domain registered to that account. An account that somehow constructs a request with `from: attacker@victim.com` must be rejected, not forwarded through SES. Check at the outbound send boundary: look up the sending account's registered domains and assert the `from` domain appears in that list. Return a 403 / reject the SFN task with a non-retryable error if it doesn't match. This is separate from DKIM/DMARC (which is DNS-level) — this is an application-layer authorisation check.

- [ ] **Switch classifier to open-weight model** — the signal classifier currently uses Claude (anthropic.claude) via Bedrock. Replace with an open-weight model (Ministral 14B or Llama 3.3 70B) for privacy and cost reasons. The task is constrained (pick from 15 workflows, extract structured fields, score spam 0–1, generate summary) — a smaller model with good instruction following should suffice. Test Ministral 14B first; fall back to Llama 70B if classification quality degrades.
...
- [ ] **Calendar proxy support** — calendar invites arrive as `.ics` attachments on any workflow's signal, not as a separate `scheduling` workflow. The full flow: (0) **Remove `scheduling` workflow**: delete from WORKFLOWS array, remove `SchedulingData` type, remove classifier prompt section, remove system rules referencing it, clean up any `workflowData` references. Emails currently classified as `scheduling` should instead be classified under their natural workflow (e.g. a meeting invite from a recruiter is `job`, from a doctor is `healthcare`, from a colleague is `crm`). (1) **Parse iCal**: detect `.ics` attachment on any signal regardless of workflow, parse into structured fields (title, start/end, location, organizer, attendees, VEVENT UID, METHOD, SEQUENCE), store raw iCal as signal attachment and parsed fields in a new `calendarData` object on the signal. (2) **Expose in UI**: signals with `calendarData` render an accept/decline card regardless of workflow. (3) **Auto-forward to user's real calendar**: on account setup, user provides their real calendar email. System forwarding rule forwards signals with iCal attachments to that address, preserving the `.ics` so their native calendar app picks it up. (4) **Receive user response**: when user clicks accept/reject in UI, store a `source: "user"` signal on the arc recording their decision. (5) **Send masked RSVP to organizer**: compose iCal REPLY (METHOD:REPLY) with user's decision, sent from alias address back to organizer — keeps real identity hidden. Key open questions: handle CANCEL and RESCHEDULE methods? Forwarding rule per-alias or account-wide? Recurring events (same VEVENT UID, different SEQUENCE)? **Separate spec needed.**
...
- [ ] **Auto-block simulated phishing / "think before you click" warning emails from service providers** — many banks and other systems now send out emails to consumers warning them about clickling malicious links or stuff to do with not giving out your password or credentials. All of these are complete noise and complete garbage and should be dealt with accordingly.
- [ ] **Promotions: extract expiry date and auto-archive when the offer lapses** — promotional emails (workflow `promotions` or `newsletter` with `workflowData.offerExpiry`) often contain a deal or discount that is meaningless after the expiry date. The classifier should extract the latest explicit expiry or validity date from the email body and store it as `workflowData.offerExpiry` (ISO-8601). A scheduled job (or TTL-based trigger) should auto-archive the arc once `offerExpiry` has passed, so the inbox is not cluttered with expired coupons. The arc should be archived (not deleted) so the user can still find it in All if needed. If no expiry date can be extracted with high confidence, do not set the field — do not guess.
- [ ] **Email forwarding loop detection and prevention** — forwarding rules can create infinite loops: if account A forwards to address X, and address X routes back into the same account (or another account that forwards back), signals will cycle forever. Two layers of protection needed: (1) **Setup-time**: when a user creates or edits a forwarding rule, check whether the rule's target address matches any domain registered to the same account — if it does, reject the rule with a clear error ("Forwarding to your own domain would create a loop"). Also check whether any existing rule already forwards *to* that same target from any label/alias that the new rule would also match. (2) **Runtime**: detect loops in-flight by inspecting the `References` / `X-Forwarded-To` / custom `X-SES-Loop` headers on inbound signals — if the message has already passed through this account or has been forwarded more than N times (e.g. 5), drop it with a non-retryable error and emit a `loop_detected` audit event. The setup-time check is the primary guard; the runtime check is the safety net for cases that slip through (e.g. two accounts that each forward to the other).
- [ ] **Validate outbound sender domain is owned by the account** — before sending any email (reply, forward, auto-reply, invite, onboarding), verify that the `from` address domain is a domain registered to that account. An account that somehow constructs a request with `from: attacker@victim.com` must be rejected, not forwarded through SES. Check at the outbound send boundary: look up the sending account's registered domains and assert the `from` domain appears in that list. Return a 403 / reject the SFN task with a non-retryable error if it doesn't match. This is separate from DKIM/DMARC (which is DNS-level) — this is an application-layer authorisation check.
...
- [ ] **Change spamScoreThreshold to 0–10 integer scale** — currently the backend uses 0–1 (float) for `spamScoreThreshold` on both Alias and AccountFilteringConfig. Switch to a 0–10 integer scale which is more intuitive for non-technical users. Update: zod validation (`min(0).max(1)` → `min(0).max(10)`), processor comparison logic (divide by 10 internally or rescale classifier output), existing stored values (migration: multiply by 10), API docs, and frontend. The classifier already produces a 0–1 float — the threshold is the user-facing knob that should be friendlier.
...
- [ ] **Complete `toApiSignal` for the signal list endpoint** — the signal list endpoint (`GET /accounts/:id/arcs/:arcId/signals`) currently returns raw DB items rather than going through `toApiSignal`. The transform has two gaps that block applying it to the list: (1) `toApiSource` maps `"signal"` and `"email"` sources to `"system"`, losing the distinction; (2) `toApiCalendarData` drops `method`, `veventUid`, and other calendar fields that the frontend and unit tests depend on. Fix both, extend `Api.SignalSource` to include all DB source values, then apply `toApiSignal` in the list endpoint so arc list and signal list both go through their respective transforms.
