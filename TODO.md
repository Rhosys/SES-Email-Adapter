...
- [ ] **Refactor SQS dispatch in handler** — the handler's `isSqsEvent` block is wrong. It should: (1) loop over all records in the event, (2) for each record, determine the type (feedback, reindex, signal, side_effect) from the record's attributes/ARN, (3) parse the body into the relevant typed payload, (4) call the relevant processor method with the parsed payload (not the raw SQSRecord), (5) on failure, add the record's messageId to batchItemFailures, (6) apply the RETRY_TRACK_THRESHOLD log-level escalation (WARN below 30, ERROR above) here in the handler — not inside each processor. This means processors accept typed payloads and return Result, not SQSEvent/SQSRecord. The threshold logic lives in one place. Also: remove `ProcessError` and its `messageId` field entirely — the handler already has the record and its messageId, it doesn't need the processor to echo it back. Processor methods return `Result<void, DbError>` (or similar), the handler maps any `err` to the corresponding record's messageId for batchItemFailures. Additionally: move the reindex API (`POST /reindex`, `GET /reindex/{jobId}`) into the Hono app as proper routes — it currently bypasses Hono and is routed in the Lambda handler directly. The job-dispatch-handler should return Result types, and the Hono route converts to HTTP responses. Remove the try/catch from the dispatcher; let errors propagate as Result values. Note: `ReindexDispatcher.dispatch()` and `getReport()` currently throw on invalid clusterId / missing job — these must return `Result<T, NotFoundError>` instead. Also: `processSideEffectRecord` currently parses `record.body` itself — the handler should parse and validate the body before passing a typed `SideEffectPayload` to the processor, eliminating the JSON.parse + validation from the processor entirely.
