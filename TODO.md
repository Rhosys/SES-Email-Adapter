...
- [ ] **Remove deprecated `statusToCategory` in favor of `statusToMetric`** — `stats-writer.ts:35` aliases `statusToCategory = statusToMetric` and is marked `@deprecated`. Migrate the 7 call sites in `processor.ts` (`:819, :939, :1142, :1237, :1259, :1352`, plus the import at `:29`) to `statusToMetric` and delete the alias.
...
- [ ] **Domain DELETE: deregister SES domain identity** — `DELETE /accounts/:id/domains/:domainId` now cascade-deletes all aliases on the domain and their sender entries (paginated lookup + parallel batch delete), soft-deletes the domain (`status: "deleted"` instead of removing the row — hidden from all reads, blocks inbound routing via `resolveAccountForRecipient`, revivable by the same account via re-POST, while other accounts remain blocked from claiming it), logs the cascade, and records audit events for both the domain and each alias deletion. Still outstanding: deregister the SES domain identity on delete.
...
- [ ] **Track alias count and approved-sender count in stats** — maintain `totalAliases` and `totalApprovedSenders` atomic counters on the account stats item. Increment sites: (1) API — POST /aliases, POST sender to approved list; (2) processor — auto-creation of a new alias on first inbound email to an unconfigured address, auto-addition of a sender to the approved list during processing. Decrement sites: API DELETE for aliases and sender removal. Expose both in `GET /accounts/:id/stats` response alongside signal counters. This gives the dashboard visibility into how much of the filtering surface the user has configured to date.
