...
- [ ] **Multiple mailboxes per provider per account** — Authress links one identity per connection per user, so `getConnectionCredentials(connectionId, connectionUserId)` cannot distinguish two Gmail mailboxes linked by the same user. `ExternalMailExchange.connectionUserId` records *who* linked a mailbox, which is enough while each user links at most one mailbox per provider; a second one silently reuses the first one's credentials. Needs either a per-EMX identity reference (if Authress exposes one) or an explicit one-mailbox-per-provider-per-user constraint enforced at activation with a clear error.
