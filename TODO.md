...
- [x] **Auto-block "think before you click" warning emails from service providers** — classified as `status:security_awareness`; SR-05 (`block_hidden`) already fires on all `system:workflow:status` signals. Added `security_awareness` to `StatusData.statusType` and the workflow registry; sharpened `status` description to explicitly name these; annotated `alert` to exclude mass-sent awareness campaigns.
