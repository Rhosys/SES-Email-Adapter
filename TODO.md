...
- [x] **Resource collapsing moved server-side** — `collapseResources` (`src/api/resource-collapse.ts`) merges resources for the same thread+workflow whose titles case-insensitively overlap and whose `displayDate`s fall on the same UTC day (a missing date on either side defers to the title match alone), unioning their `assets` and keeping the most-recently-updated display fields. Applied in `resourcesApi.ts` to both `GET /accounts/:id/resources` and `GET /accounts/:id/threads/:threadId/resources`. This supersedes the ad hoc client-side matching the UI's thread signal workflow did in `resource-match.ts`/`workflowMatchesResource` (still used there for workflow-panel-vs-resource suppression, a different concern — matching a raw signal entry against a resource, not merging resources with each other).
