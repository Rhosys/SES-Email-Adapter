...
- [x] **Create missing Alias record on quarantine approve/deny** — `POST /signals/:id/quarantineResponse` was persisting a sender disposition (`saveSender`) for brand-new addresses without ever creating the corresponding `Alias` record, unlike `processor.ts`'s `autoApprove` which creates the alias first when missing. Added `ensureAliasExists` helper in `src/api/app.ts`, wired into both the approve (`active`) and deny (`block_hidden`/`block_reject`/`violate_report`) branches.
