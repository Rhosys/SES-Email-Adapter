...
- [ ] **Expected error handling** — the side-effect handler forward path in `processSideEffect` hardcodes `dkimPass: false, dmarcPass: false` which means forwards never actually send from the side-effect path. Fix when moving forward fully to side-effects (pass verdicts through `SideEffectPayload`).
