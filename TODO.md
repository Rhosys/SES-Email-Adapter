...
- [x] **Rework Notifier** — Replaced `SesNotifier` with `DeviceNotifier`: delivers via WebSocket (all urgencies) and mobile push FCM/APNs (interrupt/ambient, suppressed when silent). Urgency passed from processor, DEVICE# records unify all targets, stale cleanup on 410/UNREGISTERED. SES notification path removed.
...
- [ ] **Replace fast-check with static `it.each` tables** — 8 test files still use `fast-check` for random input generation. Replace each `fc.assert(fc.property(...))` with `it.each(cases)` over a finite set of meaningfully different inputs. Remove the `fast-check` dependency from `package.json` after migration. Files: `spa-rewrite.test.ts`, `cluster-registry-property.test.ts`, `embedding-generator.no-throw.property.spec.ts`, `embedding-generator.result-count.property.spec.ts`, `processor.embeddings-map.property.spec.ts`, `processor.primary-failure.property.spec.ts`, `processor.primary-vector-flow.property.spec.ts`, `processor.secondary-failure.property.spec.ts`.
...
- [ ] **Move all test files to `tests/` directory** — all `*.spec.ts`, `*.test.ts`, and test helpers (`src/testing/`, `src/processor/test-helpers.ts`) move to a top-level `tests/` tree that mirrors `src/`. Production code lives exclusively in `src/`; test code lives exclusively in `tests/`. Structure: `tests/processor/processor.spec.ts`, `tests/helpers/mock-logger.ts`, etc. Update vitest config to find tests in `tests/`, update tsconfig to include `tests/` for type-checking, update all import paths.
- [ ] ** Fixup Signal, views, rules, arc, etc.. ALL genrated IDs** IDs should always be generated similar to, but instead of a acc- it should be a sgn, view-, rule-, arc-, etc... Signal might be a bit weird because we are PK it from the SES messageId right? I wonder what we can do there, I wonder if there should be an internal and external mapping for it? Or if I did fuck us ealier by making the sesMessageId be the PK
```
