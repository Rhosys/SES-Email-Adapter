...
- [ ] **Multiple mailboxes per provider per account** — `getConnectionCredentials(connectionId, userId)` is keyed on the connection and the Authress user, so two Gmail mailboxes linked by the same user under the same connection resolve to the same credentials. The exchange now records `connectionUserId` (the provider-side identity id), which is enough to *detect* the collision — two exchanges on one connection with different `connectionUserId` values cannot both be served. Either reject the second at activation with a clear error, or find out whether Authress can vend credentials per linked identity rather than per connection.
