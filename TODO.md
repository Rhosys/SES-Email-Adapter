...
- [ ] **Never wrap a `DbError` in another `dbError()`** — `dbError(e)` is correct when `e` is a raw exception, a string, or a different error type that needs to be converted to `DbError` at a type boundary. It is wrong when `e` is already a `DbError`: the kind is already correct, the extra wrap adds a `{kind:db_error, cause:{kind:db_error, cause:...}}` layer that obscures the real cause in logs and makes serialized errors unreadable. When propagating a `DbError` upward, pass it through directly: `if (r.isErr()) return err(r.error)`. Only use `dbError()` to convert something that is not yet a `DbError`.
