...
- [ ] **Prefix all outbound SES tags with `X-Numaeel-` and include signalId + arcId** — all outbound emails (forward, reply, pong) currently use bare tag names (`type`, `accountId`). Rename to `X-Numaeel-Type`, `X-Numaeel-AccountId`, and add `X-Numaeel-SignalId` and `X-Numaeel-ArcId` where available. "Numaeel" is the internal codename (swappable later). This improves traceability in SES feedback notifications — the feedback processor can correlate bounces/complaints back to the exact signal and arc without a DB lookup.
- [ ] **Replace `saveArc` with `updateArc` for partial updates** — audit all call sites that call `store.saveArc(arc)` when they only need to change one or two fields (status, labels, urgency, lastSignalAt). Convert them to `store.updateArc(accountId, arcId, { ...fields })` which does a targeted DynamoDB UpdateExpression. `saveArc` should only be used for initial arc creation. This reduces write amplification and makes intent clearer at each call site.
- [ ] **Replace all `new Date().toISOString()` with Luxon `DateTime.utc().toISO()`** — audit every call site that uses `Date` for timestamp generation and convert to `DateTime.utc().toISO()` from Luxon. This ensures all stored dates are explicitly UTC ISO 8601 strings with no ambiguity about timezone. `new Date().toISOString()` happens to produce UTC, but Luxon makes the intent explicit and provides better parsing/formatting for any future date arithmetic. Add `luxon` as a dependency if not already present.
...
- [ ] **Calendar invite interception and response flow** — calendar invites are `scheduling` workflow signals with `eventType: "meeting_invite"`, not a separate workflow (an invite can arrive inside any conversation arc). The full flow: (1) **Parse iCal**: extract the `.ics` attachment from the signal, parse it into structured `SchedulingData` fields (title, start/end, location, organizer, attendees, VEVENT UID), store the raw iCal as a signal attachment. (2) **Expose in UI**: enrich `SchedulingData` with explicit fields the UI needs — `icalUid`, `method` (REQUEST/CANCEL/REPLY), `sequence`, `attendeeStatus` (needs-action/accepted/declined). The UI renders an accept/decline card. (3) **Auto-forward to user's real calendar**: on account setup, the user provides their real calendar email (e.g. personal Gmail). Create a system forwarding rule that forwards scheduling signals with `eventType: "meeting_invite"` to that address, preserving the iCal attachment so their native calendar app picks it up. (4) **Receive user response**: when the user clicks accept/reject in our UI, store a `source: "user"` signal on the arc recording their decision. (5) **Send masked RSVP to organizer**: compose an iCal REPLY (METHOD:REPLY) with the user's decision, sent from the alias address (not the user's real email), back to the organizer. This keeps the user's real identity hidden while participating in the calendar flow. Key open questions: should we also handle CANCEL and RESCHEDULE methods? Should the forwarding rule be per-alias or account-wide? How do we handle recurring events (same VEVENT UID, different SEQUENCE)? **Separate spec needed.**
