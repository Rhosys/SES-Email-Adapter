...
- [ ] **Fix Authress invite-accept to amend the account's existing AccessRecord, not create a separate one** — `AuthressAccessService.createInvite` (`src/api/authress-access.ts:163-175`) builds an ad-hoc `statements` array on the invite itself instead of targeting the account's canonical record (`recordId(accountId) = "email:account-${accountId}"`, same file line 65-67). Every other mutation path (`_upsertUser`, `listUsers`, `removeUser`) reads/writes that single named record. Need to verify with Authress whether accepting an invite merges into an existing `recordId` or always creates a new AccessRecord — if it's the latter, an accepted invitee may not show up in `GET /accounts/:id/users` or be manageable via the existing PATCH/DELETE user endpoints, since those only ever look at `email:account-${accountId}`. Also fix: `email` is accepted as a param but never included in the actual `client.invites.createInvite()` payload, so the invite isn't tied to the recipient's address on the Authress side. While in there, `renderTemplate("team-invite", ...)` (`src/api/accountsApi.ts:291`) never supplies `inviterName`/`inviterEmail`/`accountId`, which `team-invite.mjml` references directly — those fields render blank in every invite email today.
...
- [ ] **Pending invites: list + revoke** — `POST /accounts/:id/users` creates an Authress invite but nothing persists or exposes it afterward; there's no way to see who's been invited but hasn't accepted, or to cancel a bad invite. Add `GET /accounts/:id/invites` (list pending, not-yet-accepted invites) and `DELETE /accounts/:id/invites/:inviteId` (revoke). Matches the already-documented UX requirement below ("Pending invites section... with resend / revoke options") which currently has zero backend support.
