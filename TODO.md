...
- [ ] **OnboardingTaskHandler should not auto-mark completed** — the Step Function followup handler currently sets `onboarding.completed = true` when all milestones pass. Now that the user explicitly clicks "Done" in the UI to mark completion, the SFN should only set `testEmailReceived: true` (if signals exist and it isn't set yet). The `completed` flag is exclusively user-controlled via the Done button.
