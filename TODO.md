...
- [ ] **Domain DELETE: clean up SES domain identity + block incoming email** — `DELETE /accounts/:id/domains/:domainId` now cascade-deletes all aliases on the domain and their sender entries (paginated lookup + parallel batch delete), logs the cascade, and records audit events for both the domain and each alias deletion. Still outstanding: deregister the SES domain identity on delete, and permanently block incoming emails for deleted domains (reject at SES receipt rule level or via a deny-list check in the processor).
