...
- [ ] **Ask Authress to support targeting an explicit `recordId` on invite creation** — confirmed (per Authress SDK types, `src/invites/dtos.d.ts`) that `Invite` has no `recordId` field; Authress always attempts an automatic "safe merge" into an existing matching AccessRecord first, falling back to `conflictResolutionStrategy` (default `GENERATE_NEW_RECORD`) when that's not possible. In practice, accepted invites currently land in a separate, Authress-generated AccessRecord rather than our canonical `email:account-${accountId}` record (`recordId()`, `src/api/authress-access.ts`), so `removeUser`/`updateUserRole` — which only mutate that one named record — can't guarantee they actually revoke/change an invited user's access. File a feature request with Authress for explicit `recordId` targeting on `createInvite()`, so invite-accept can merge directly into our canonical record instead of relying on undocumented auto-merge behavior. **Interim mitigation shipped**: `createInvite()` now sets `conflictResolutionStrategy: "SKIP_CHANGES"` (the only fallback that can't over-grant or destroy existing team access — worst case is the invite silently grants nothing), `listUsers()` now reads via `resources.getResourceUsers(accounts/{accountId})` which aggregates across all AccessRecords rather than just the canonical one, and `PATCH /accounts/:id/users/:userId` now emits a TRACK log (`accounts.user_role_updated`) on every role change so a discrepancy is investigable. `removeUser`/`updateUserRole` still only mutate the canonical record — full guarantee blocked on the Authress feature request above.
