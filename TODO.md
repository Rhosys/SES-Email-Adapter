...
- [ ] **Create skill: API parameter design — no booleans, orthonormal basis** — document the rule that API and configuration parameters must never use booleans, and that the full parameter set must form an orthonormal basis. A boolean collapses a dimension to two states and can never be extended without a breaking change; replace with an enum whose values name the distinct states explicitly. Orthonormal basis means: (1) orthogonal — each parameter controls exactly one independent dimension with no overlap or redundancy between parameters (e.g. `all` as an enum value is not orthogonal — it is the union of other values and adds no new expressive power); (2) normal / complete — the parameters together span the full configuration space, with each distinct configuration reachable by exactly one combination of parameter values. Violations to flag: boolean flags that encode a choice (`enabled: true/false` → replace with an explicit enum); catch-all shorthand values that duplicate combinations of other values (`all`, `none`, `default`); parameters whose meaning changes depending on another parameter's value (implicit coupling). This applies to REST API bodies, Terraform resource arguments, environment variables, and database/RDS parameter groups.
