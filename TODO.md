...
- [ ] **Untrusted-sender fallback reads the matched thread's stale labels** — in `processInbound` (`src/processor/incoming-email-processor.ts`) the unknown-sender fallback checks `thread.labels.includes("system:sender:untrusted")`, but `thread.labels` is the matched thread's persisted labels merged with this signal's, and labels are only ever added. A thread that once carried `system:sender:untrusted` (e.g. archived by a rule before the sender was approved) keeps it, so later mail from the now-allowed sender that matches that thread is re-quarantined. Quarantine approvals are immune (they replay with `userApproved`), ordinary inbound is not. Decide the fallback off the freshly computed `systemLabels` and drop the stale label from the thread when the sender is trusted; SR-01's `thread.labels` condition needs the same review.
- [ ] **RSVP reminder / calendar_response lookups aren't RECURRENCE-ID-aware** — `getLatestCalendarResponse` (`src/database/thread-database.ts`) and `rsvp-reminder-handler.ts` match a thread's `calendar_response` signals purely by `veventUid`. Since `findCalendarAttachments`/`collapseCalendarEvents` (`src/processor/calendar/ics-parser.ts`) now emit a separate `calendar_event` signal per UID+RECURRENCE-ID occurrence exception, two different occurrences of the same recurring series share a `veventUid` and could have their RSVP responses cross-matched. Scope: thread the RECURRENCE-ID through the RSVP/response lookup path the same way `buildCalendarSignalLookupId` now does for signal storage.
- [ ] **Verify `calendar-collapse.ts`'s cross-email history premise against actual signal storage** — `collapseCalendarSignals` (`src/api/calendar-collapse.ts`) assumes a thread can hold multiple stored `calendar_event` signals for the same event over time (original invite, updates, cancellation) and collapses them at read time. But `saveSignal` (`src/database/thread-database.ts`) writes calendar signals keyed by `pk = ACCT#{accountId}#SIG#{signalLookupId}` with a constant `sk`, and `signalLookupId` for calendar signals is `buildCalendarSignalLookupId(organizer, veventUid[, recurrenceId])` — stable across an event's lifetime. That looks like each new email for the same event **overwrites** the prior stored row rather than appending a new one, which would mean the read-time collapse logic can never actually see more than one signal per event group. Confirm which behavior is real (append vs. overwrite) and reconcile: either the write path needs to append (e.g. include something time/version-varying in the lookup key or switch to a real history table) or the read-time collapse logic is dead code that can be simplified/removed.

- [ ] **Remove `resolveImapSyncState()`'s transitional `syncCursor`-parsing fallback (optional cleanup, not a deprecation)** — `syncCursor` is not going away: IMAP writes it on every `activate()`/`renew()`/PATCH alongside the structured `syncState` property bag, as a permanent human-readable parallel (surfaced as-is in the external-exchanges API response). The only thing that's transitional is the *read-side fallback*: `resolveImapSyncState()` (`src/external-exchanges/imap-adapter.ts`) reads `syncState` first and only falls back to parsing the colon-delimited `syncCursor` string for exchanges written before `syncState` existed. Every active IMAP exchange gets `syncState` backfilled the next time it goes through `renew()` (15-min sweep) or a PATCH/connection-test. Once a deploy has been live for a full cycle (24h is generous — comfortably covers exchanges that needed a retry or two), audit DynamoDB for any *active* IMAP exchange missing `syncState`; if none remain, the fallback branch is provably dead and can be deleted from `resolveImapSyncState()` and its test coverage in `tests/external-exchanges/imap-adapter.test.ts` — `syncCursor` itself and its writes stay untouched. JMAP/Gmail/Outlook are unaffected — their `syncCursor` is a genuinely opaque single-token cursor from the provider, not a local encoding, so they don't have (or need) a `syncState` bag at all.
...
- [ ] **User attachment upload for outbound signals (replies/compose)** — drafts currently have `attachments: []` hardcoded; no upload endpoint exists. Needed: `POST /accounts/:accountId/attachments` → presigned S3 PUT URL + `attachmentId`; client uploads directly to S3; draft creation/update references `attachmentId` values; send logic fetches from S3. TTL-based cleanup for unreferenced uploads. **Now covered by the shared `BlobService` design in `.kiro/specs/jmap-support/` (Task 9)** — built alongside JMAP's blob upload/download rather than as a separate, disconnected feature.
...
- [ ] **JMAP support (RFC 8620 + RFC 8621 + RFC 9404)** — standards-compliant JMAP server, OAuth2-only auth via Authress (using Authress's own OAuth endpoints directly, no proxy routes), Mailbox/Thread/Email/Identity/EmailSubmission mapped onto the existing Account/Signal/Alias/Domain/Thread model. `Foo/changes` derived from existing `Thread`/`Signal` `updatedAt` fields, no new change-log table. **Active spec: `.kiro/specs/jmap-support/`**
