...
- [x] **Classify "think before you click" warning emails as `notice:security_awareness`** — added `security_awareness` to `NoticeData.noticeType` and the workflow registry; sharpened the `notice` description to explicitly name mass-sent phishing warnings and credential-safety reminders; annotated `alert` to exclude mass-sent awareness campaigns. NOTE: these are no longer auto-blocked — the blanket "block all notice emails" system rule was removed (see decision below), so security_awareness emails now land in the inbox with their push notification suppressed (SR-06) and silent urgency. If outright blocking of `security_awareness` specifically is wanted, add a dedicated system rule keyed on a `system:notice:security_awareness` label.
...
- **`notice` workflow** arcs reach the inbox but are quiet — push notification suppressed by default (SR-06) and silent urgency (`priority.ts`). They are no longer auto-blocked.
