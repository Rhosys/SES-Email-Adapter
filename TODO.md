...
- [x] ~~New `resources` table for time-based/dated workflow items~~ — **Shipped.** New `resources` DynamoDB table (`deploy/storage.tf`) + `src/database/resource-database.ts` + `src/processor/resource-info.ts` (`deriveResourceInfo`), wired into `SignalProcessor` right after signal save. `pk = ACCT#{accountId}#THREAD#{threadId}`, `sk = {workflow}#{resourceKey}` (deterministic — a later signal for the same order/flight/invoice/etc. overwrites the same item, no read-before-write; `resourceKey` is a per-workflow natural identifier, one level finer than the existing thread `groupingKey`). `gsi1pk = ACCT#{accountId}#STATUS#{status}#WORKFLOW#{workflow}`, `gsi1sk = expectedResolutionDate` — native date-range queries (e.g. "packages arriving in the next 3 days") with no app-side filtering. Public API (`GET /accounts/:id/resources`, `GET /accounts/:id/resources/:resourceId`) encodes `threadId` directly into the opaque `resourceId` so by-id lookups need no secondary index. Covers `package`/`travel`/`payments`/`healthcare`/`job`/`events`. Completion is signal-driven only (explicit terminal `workflowData` values) — **no scheduled sweep**, so `travel` resources (which have no data-derivable "trip completed" state) stay `active` until their TTL expires; a completion mechanism for date-passed-with-no-terminal-signal resources is intentionally deferred, not built. Frontend does not yet consume this API — the original "stacked / today's delivery" UI motivating this table is still TODO. Long-term: revisit whether `package`/`travel`/`payments`/`healthcare`/`job`/`events` should collapse into fewer `Workflow` enum values now that they share one underlying "dated resource" mechanic — the distinction may only be display/copy, not structural.
- [ ] **Resources: UI consumption** — build the "stacked" per-workflow view and "today's delivery alert" banner against the new `GET /accounts/:id/resources` endpoint (see shipped item above). This was the original product motivation for the `resources` table.
- [ ] **Resources: date-passed auto-completion** — resources with no data-derivable terminal state (currently only `travel`) never transition to `complete` on their own. Decide on a mechanism (scheduled sweep, lazy-complete-on-read, etc.) once there's a concrete UI need to distinguish "still active" from "date passed, presumably done."
