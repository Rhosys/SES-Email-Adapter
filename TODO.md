...
- [ ] **Never double-wrap errors** — when a called function already returns a typed `Result` error (e.g. `DbError`), propagate it directly: `if (r.isErr()) return err(r.error)`. Do NOT re-wrap with `err(dbError(r.error))` — that creates a `DbError` whose `cause` is another `DbError`, which is semantically wrong (the outer code does not know the inner failure was a database issue), destroys useful context in error logs, and makes `JSON.stringify` output deeply nested objects that obscure the real failure. `dbError()` is only correct in two places: (1) inside a `catch` block wrapping an SDK/library exception at the outermost database boundary, and (2) when intentionally converting a different error kind (e.g. `InvalidResponseError → DbError`) at a type boundary. Anywhere else is a bug.
