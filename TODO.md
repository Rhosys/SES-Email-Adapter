...
- [ ] **Remove legacy IMAP `syncCursor` string fallback** — `resolveImapSyncState()` (`src/external-exchanges/imap-adapter.ts`) reads the structured `syncState` property bag first, falling back to parsing the old colon-delimited `syncCursor` string (`"{uidvalidity}:{lastUid}"`) for exchanges created before `syncState` existed. Every active IMAP exchange gets `syncState` backfilled the next time it goes through `renew()` (15-min sweep) or a PATCH/connection-test — both write it unconditionally now. Once a deploy has been live for a full cycle (renewal sweep runs every 15 min, so 24h is generous — comfortably covers exchanges that were failing renewal and needed a retry cycle or two), audit DynamoDB for any *active* IMAP exchange missing `syncState`; if none remain, delete the legacy-cursor branch of `resolveImapSyncState()`, its test coverage in `tests/external-exchanges/imap-adapter.test.ts`, and the `syncCursor` legacy-format comment on `ExternalMailExchange` in `src/types/index.ts`. JMAP/Gmail/Outlook are unaffected — their `syncCursor` is a genuinely opaque single-token cursor from the provider, not a local encoding, so they keep using it indefinitely.
