...
- [x] **Multiple mailboxes per provider per account** — `@authress/sdk@3.2.280` added a third `connectionUserId` parameter to `getConnectionCredentials(connectionId, userId, connectionUserId)`, which selects a specific linked identity's stored credentials instead of always returning the latest login flow's. Threaded through: `ActivationIdentity`/`exchangeCredentials()` now carry `connectionUserId`; `GmailProvider`/`OutlookProvider`'s `resolveToken`/`activate` and their `getProviderToken` deps take it as a third arg; `composite-root.ts`'s `getProviderToken` forwards it to the SDK call. On the create path, `connectionUserId` is now a required request field, and `AccessService.getLinkedIdentity(userId, connectionId, connectionUserId)` validates that the exact pair is one of the caller's linked identities (rather than `.find()`-ing the first match) before it is trusted or persisted — a mismatch is refused with 403 and a TRACK log, not silently substituted. **Still open**: the UI side of picking which `connectionUserId` to send when a connection already has multiple linked identities — see below.
- [ ] **UI: disambiguate `connectionUserId` when a connection has multiple linked identities** — the backend now requires and validates a specific `connectionUserId` on mailbox connect (see above), but the UI's `completeExchangeActivation()` still resolves it via `getUserProfile().linkedIdentities.find(...)`, which has the same "first match wins" bug the backend used to have. This is fine for a single mailbox per connection but picks the wrong identity once a user has linked more than one mailbox through the same provider connection. Needs a UI-side way to know which identity was just linked by the in-flight `linkIdentity()` redirect (e.g. a return-flow marker) rather than searching the full linked-identities list after the fact.
