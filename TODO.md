...
- [ ] **Spam score threshold must be user-configurable, not hardcoded** — currently `isSpam = classification.spamScore >= 0.9` is hardcoded in `processor.ts`. This belongs in account filter config so users can tune it:
  ...
  - Per-address config can override it, same as `filterMode`
  - `blockDisposition.spam` already controls what happens when the threshold is crossed (block vs quarantine) — that pairing is correct, just the threshold needs to move out of the code and into config
  - Surface in UI as a slider or stepped selector (e.g. Strict / Balanced / Lenient) in Settings → Account → Filtering, with plain-language descriptions of what each level means in practice
