...
- [ ] **Ask Authress to support targeting an explicit `recordId` on invite creation** — confirmed (per Authress SDK types, `src/invites/dtos.d.ts`) that `Invite` has no `recordId` field; Authress always attempts an automatic "safe merge" into an existing matching AccessRecord first, falling back to `conflictResolutionStrategy` (default `GENERATE_NEW_RECORD`) when that's not possible. In practice, accepted invites currently land in a separate, Authress-generated AccessRecord rather than our canonical `email:account-${accountId}` record (`recordId()`, `src/api/authress-access.ts`), so `removeUser`/`updateUserRole` — which only mutate that one named record — can't guarantee they actually revoke/change an invited user's access. **Key open question for Authress**: does the "safe merge" / `conflictResolutionStrategy` fallback logic ever consider a record we created ourselves via `accessRecords.createRecord` (like our canonical record) as a merge candidate, or does it only ever match against records Authress itself previously auto-generated for that resource/invite? If it's the latter, our canonical record is never a valid merge target under any `conflictResolutionStrategy`, and explicit `recordId` targeting on `createInvite()` is the only real fix. File a feature request with Authress for that. **Interim mitigation shipped**: `createInvite()` sets `conflictResolutionStrategy: "UNSAFE_FORCE_MERGE"` — our records are always one statement per role with a `users` list (see `_upsertUser`), so merging a new user into the matching role's statement can't change what any other existing user is granted, making a forced merge safe for our specific record shape (unlike the general "may over-grant other users" risk the option name implies). `listUsers()` reads via `resources.getResourceUsers(accounts/{accountId})`, which aggregates across all AccessRecords rather than just the canonical one, and `PATCH /accounts/:id/users/:userId` emits a TRACK log (`accounts.user_role_updated`) on every role change so a discrepancy is investigable. `removeUser`/`updateUserRole` still only mutate the canonical record — full guarantee blocked on the Authress feature request above.
- [ ] **`GET /users/{userId}` endpoint (top-level, not account-scoped)** — returns display name + profile picture for a given userId, backed by the existing `AccessService.getUserProfile(userId)` (already used internally by `GET /accounts/:id/users` to enrich team member rows). Needed so the UI can resolve a bare `userId` (e.g. from an audit log entry, or any per-account resource showing "who did this") into a display name/avatar without needing account-membership context. Auth: any authenticated caller, no account-scoped authz — it only exposes name/picture, no secrets.
