...
- [x] **Create missing Alias record on quarantine approve/deny** — `POST /signals/:id/quarantineResponse` was persisting a sender disposition (`saveSender`) for brand-new addresses without ever creating the corresponding `Alias` record, unlike `processor.ts`'s `autoApprove` which creates the alias first when missing. Extracted the shared logic into `AccountDatabase.ensureAlias(accountId, address, defaultUnknownSenderPolicy, existing?)`; `processor.ts`'s `autoApprove` and `src/api/app.ts`'s `ensureAliasExists` helper both delegate to it instead of duplicating the alias-construction logic.
