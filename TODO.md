...
- [ ] **Block emails that fail DKIM or DMARC** — if either verdict is not PASS, the signal gets `block_reject` status immediately. No arc created, no side-effects dispatched. This eliminates `ForwardOptions` from the forwarder — by the time we forward, the message already passed authentication. Remove the DKIM/DMARC gate from `SesForwarder.doForward`, remove the inline forward/auto-reply/auto-draft from `processMessage` (only `processSideEffect` should execute them), and delete `ForwardOptions` entirely.
