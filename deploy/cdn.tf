...
resource "aws_s3_bucket" "web" {
  bucket           = "${lower(var.service_name)}-web-${var.aws_account_id}-${local.primary_region}-an"
...
resource "aws_s3_bucket" "landing" {
  bucket           = "${lower(var.service_name)}-landing-${var.aws_account_id}-${local.primary_region}-an"
  bucket_namespace = "account-regional"
}
