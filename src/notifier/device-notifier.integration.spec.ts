...
import type { Signal, Arc, Alias, ArcUrgency } from "../types/index.js";
...
describe("DeviceNotifier wiring: handler instantiates with correct dependencies", () => {
  it("processor options include a notifier with a 4-argument notify method", async () => {
    ...
    const { DeviceNotifier } = await import("./device-notifier.js");
    const { DynamoDeviceStore } = await import("./device-store.js");
    const { WsDeliverer } = await import("./ws-deliverer.js");
    const { FcmDeliverer } = await import("./fcm-deliverer.js");
