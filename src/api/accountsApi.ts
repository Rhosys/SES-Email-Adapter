...
export class AccountsApi {
  ...
  register(app: OpenAPIHono<AppEnv>, { authz, err, route }: RouteHelpers): void {
    ...
    app.openapi(route({
      ...
      const sendResult = await emailService.send({ to: [body.email], subject: `You've been invited to join ${accountName} on Numaeel`, textBody, htmlBody, sendType: "account-invite", tags, fromSender: platformFromAddress("notifications", MAIL_DOMAIN), accountId: emailService.platformTenant });
      if (sendResult.isErr()) {
