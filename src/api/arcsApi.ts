...
import type { SignalReprocessor } from "./app.js";
import {
...
import type { AppEnv, ListArcsParams } from "./app.js";

type ErrorCodeLiteral = z.infer<typeof ErrorCode>;

export interface ArcsApiDeps {
  arcDb: ArcDatabase;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function registerArcsRoutes(app: OpenAPIHono<any>, deps: ArcsApiDeps): void {
  const { arcDb, accountDb, logger, draftSendDispatcher, schedulerClient, emailService, rsvpComposer, postApprovalCalendarDeps, signalReprocessor, s3Client, emailBucket, contentCdnBaseUrl, authz, err, route } = deps;
