...
export class ThreadsApi {
  ...
  register(app: OpenAPIHono<AppEnv>, { authz, err, route }: RouteHelpers): void {
    ...
    app.openapi(route({
      ...
      logger.info("Thread updated", { code: "api.threads.patched", accountId, threadId, changedFields });
      return c.json(toApiThread(updateResult.value), 200);
    });
    ...
    app.openapi(route({
      ...
      const threadId = c.req.param("threadId")!;
      const signalId = c.req.param("id")!;
      logger.info("Sending draft signal", { code: "api.threads.send", accountId, threadId, signalId });
      ...
      // Re-validate address format (cheap, in-process) at the point the draft actually goes out
      // — schema validation at create/replace/patch time covers the common path, but this
      // catches drafts written before that validation existed and any other way an address could
      // still be malformed. Same syntax check SES itself would apply ("Illegal address"). MX/DNS
      // validation is a real network lookup per domain — too expensive for this request/response
      // cycle — so it runs in DraftSendWorker, off the request path, right before the SES call.
    ...
    app.openapi(route({
      ...
      logger.info("Thread unsubscribed and archived", { code: "api.threads.unsubscribed", accountId, threadId, unsubscribeType: unsubscribe.type });
