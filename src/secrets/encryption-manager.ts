...
export class EncryptionManager {
  ...
  async init(): Promise<void> {
    ...
    if (result.isErr()) {
      this.logger.warn("[EncryptionManager] KMS init failed — operations will retry on first use", { code: "encryption.init_failed", levelThreshold: 20, error: result.error });
    }
  }
