...
export function makeAppDeps(overrides: Partial<AppDeps>): AppDeps {
  return {
    ...
    logger: {} as never,
    verificationMailer: { sendForwardVerification: async () => ok(undefined), sendCalendarForwardVerification: async () => ok(undefined), verifyWebhookTarget: async () => ok(undefined) } as never,
