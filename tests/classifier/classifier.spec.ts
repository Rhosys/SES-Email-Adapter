...
describe("SignalClassifier", () => {
  ...
  // -------------------------------------------------------------------------
  // classify — action-required notices should be alert, not notice
  // -------------------------------------------------------------------------

  describe("action-required service emails (alert, not notice)", () => {
    it("classifies a copyright takedown requiring content removal as alert", async () => {
      mockClassifyResponse({
        workflow: "alert",
        workflowData: {
          workflow: "alert",
          alertType: "compliance_action",
          service: "Acme Hosting",
          severity: "warning",
          requiresAction: true,
          actionUrl: "https://acmehosting.example/complaints",
        },
        tags: [],
        summary: "Copyright complaint — remove reported content within 24 hours.",
        labels: [],
      });
      ...
      const result = await classifier.classify({
        ...
        receivedAt: "2024-06-10T11:30:00Z",
      ...
      const output = result._unsafeUnwrap();
      expect(output.workflow).toBe("alert");
      expect(output.workflowData).toMatchObject({ alertType: "compliance_action", requiresAction: true });
      expect(output.tags).toEqual([]);
    });

    it("classifies an account-frozen notice requiring unlock action as alert", async () => {
      mockClassifyResponse({
        workflow: "alert",
        workflowData: {
          workflow: "alert",
        ...
        tags: [],
        summary: "Account frozen due to unresolved copyright complaint — action required.",
        labels: [],
      });
      ...
      const result = await classifier.classify({
        ...
        receivedAt: "2024-06-11T11:35:00Z",
        headers: { "authentication-results": "spf=pass dkim=pass dmarc=pass" },
        allowedLabels: [],
      });
