...
describe("PATCH /accounts/:id – digest forwardingTargetId validation", () => {
  it("returns 422 when forwardingTargetId references unverified target", async () => {
    ...
    const app = createApp(makeAppDeps({
      ...
      verificationMailer: { sendForwardVerification: vi.fn().mockResolvedValue(ok(undefined)), sendCalendarForwardVerification: vi.fn().mockResolvedValue(ok(undefined)), verifyWebhookTarget: vi.fn().mockResolvedValue(ok(undefined)) },
