...
describe("Property 2: Deterministic per-day healthcheck identity", () => {
  describe("subject is 'Healthcheck YYYY-MM-DD' for multiple dates", () => {
    ...
    it.each(dateCases)("$label ($date) → subject matches expected format", async ({ date }) => {
      ...
      const sendArgs = sendSpy.mock.calls[0]![0] as { subject: string; textBody: string; headers: Array<{ Name: string; Value: string }> };
      ...
      // A tag-safe healthcheck id is attached as a custom MIME header (auto-promoted to SES tag).
