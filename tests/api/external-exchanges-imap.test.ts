...
function makeImapAdapter(): ProviderAdapter {
  return {
    activate: vi.fn().mockResolvedValue(ok({ syncCursor: "123:42", expiresAt: "2025-06-15T11:00:00Z", providerSubscriptionId: "poll" })),
...
function makeAccountDb() {
  return {
    ...
    setAliasExchange: vi.fn().mockResolvedValue(ok({ id: "a", accountId: TEST_ACCOUNT_ID, aliasAddress: "user@example.com", domain: "example.com", aliasName: "user", unknownSenderPolicy: "allow_all", createdAt: "", updatedAt: "" })),
  };
}

function makeEncryptionManager() {
  return {
    encrypt: vi.fn().mockReturnValue("encrypted-blob-base64"),
