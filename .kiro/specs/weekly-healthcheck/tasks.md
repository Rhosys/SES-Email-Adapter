...
- [ ] 1. Prerequisite A: Multi-row embeddings in Aurora pgvector
  - [ ] 1.1 Update Aurora schema and processor INSERT logic
  ...
  - [ ] 1.2 Update ThreadMatcher similarity search to deduplicate by threadId
    ...
    - _Requirements: A.3_
  - [ ] 1.3 Add `hasEmbedding(threadId)` method to ThreadMatcher
    - Query `thread_embeddings` for existence of at least one row with the given threadId
    - Return boolean
    - _Requirements: A.4, A.5_
  - [ ]* 1.4 Write unit tests for multi-row embedding changes
...
- [ ] 2. Prerequisite B: Test detection fix — eTLD+1 comparison
  - [ ] 2.1 Replace `getDomainByName` with `listDomains` + eTLD+1 comparison in processor
  ...
  - [ ]* 2.2 Write unit tests for eTLD+1 comparison logic
    - Test subdomain matching (e.g. `mail.example.com` matches `example.com`)
    - Test non-matching domains
    - _Requirements: B.3, B.4_

- [ ] 3. Prerequisite C: assign_workflow rule action updates signal workflow
  - [ ] 3.1 Propagate assign_workflow to signal data after rules evaluation
  ...
  - [ ]* 3.2 Write unit tests for assign_workflow propagation
...
- [ ] 4. Prerequisite D: SYSTEM account workflow override
  - [ ] 4.1 Add SYSTEM account workflow override in processor
  ...
  - [ ]* 4.2 Write unit tests for SYSTEM account override
...
- [ ] 5. Prerequisite E: Refactor duration conversion to use luxon
  - [ ] 5.1 Replace `DURATION_SECONDS` lookup in `src/processor/retention.ts` with luxon
  ...
  - [ ] 5.2 Replace `retentionDurationToSeconds` in `src/embedding/retention-tier.ts` with luxon
    - Same pattern: `Duration.fromISO(duration).as('seconds')`
    - _Requirements: E.4_
  - [ ] 5.3 Replace `DURATION_MS` lookup in `site-ui/src/lib/retention.ts` with luxon
    - Use `Duration.fromISO(duration).toMillis()` instead of manual map
    - Handle `"Infinity"` as a special case (return null) before calling luxon
    - _Requirements: E.5_
  - [ ] 5.4 Replace inline `90 * 24 * 60 * 60` hardcoded TTLs in `src/processor/processor.ts`
    - Use `Duration.fromISO("P90D").as('seconds')` or equivalent
    - _Requirements: E.6_
  - [ ]* 5.5 Write unit tests for luxon-based duration conversion
...
- [ ] 6. Checkpoint — Ensure all prerequisite tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Add healthcheck workflow type and classifier prompt
  - [ ] 7.1 Add `"healthcheck"` to WORKFLOWS array and HealthcheckData interface
...
- [ ] 8. Create SystemAccountDb class
  ...
  - [ ]* 8.2 Write unit tests for SystemAccountDb
...
- [ ] 9. Wire AccountDatabase delegation to SystemAccountDb
  - [ ] 9.1 Add SystemAccountDb delegation to AccountDatabase methods
  ...
  - [ ]* 9.2 Write unit tests for AccountDatabase delegation
...
- [ ] 10. Create HealthcheckJob class
  - [ ] 10.1 Implement HealthcheckJob with validate and send phases
  ...
  - [ ]* 10.2 Write property tests for HealthcheckJob (parameterized example-based)
  ...
  - [ ]* 10.3 Write property tests for graceful degradation
  ...
  - [ ]* 10.4 Write property tests for validation failure detection
...
- [ ] 11. Create healthcheck.mjml email template
  - [ ] 11.1 Create MJML template for healthcheck email
...
- [ ] 12. Handler routing changes
  - [ ] 12.1 Update EventBridge routing in handler.ts for healthcheck and unknown rules
  ...
  - [ ]* 12.2 Write property tests for handler routing
...
- [ ] 13. Wire HealthcheckJob in handler.ts
  - [ ] 13.1 Instantiate HealthcheckJob and connect to routing
...
- [ ] 14. Checkpoint — Ensure all feature tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 15. Terraform resources for EventBridge scheduling
  - [ ] 15.1 Add EventBridge rule, target, and permission to storage.tf
