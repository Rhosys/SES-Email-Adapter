...
4. WHEN the audit examines the `violate_report` flow, THE Audit SHALL identify that the arc is updated to `deleted` status but the sender block is written to a different alias address than the one that received the signal, if the arc contains signals from multiple recipient addresses.
